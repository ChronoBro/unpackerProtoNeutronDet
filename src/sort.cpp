...
int main(int argc,char *argv[])
{
  ...
  for (;;)  // loop over run numbers
      ...
      for (int iExtra=0;iExtra<3;iExtra++) //loop over split evtfiles
	{
	  ...
	  if(number < 10)
	      outstring << "/data1/protoNeutronDetector/run-000" << number;
	  else if(number >=10 && number < 100)
	    outstring << "/data1/protoNeutronDetector/run-00" << number;
	  else 
	    outstring << "/data1/protoNeutronDetector/run-0"<<number;
	 ...
	 for(;;)  // loop over items in a evtfile
		...
		*/
		...
		Det.Detector->eventNum = physicsEvent;
  ...
  // cout << "Number of cross-talk R21-R22 = " << Det.Detector->Ncross << endl;
